...
var daemonCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		...
		ShortDescription: `
'mefs-keeper daemon' runs a persistent mefs-keeper daemon that can serve commands
over the network. Most applications that use MEFS will do so by
communicating with a daemon over the HTTP API. While the daemon is
running, calls to 'mefs-keeper' commands will be sent over the network to
the daemon.
`,
		LongDescription: `
The daemon will start listening on ports on the network, which are
documented in (and can be modified through) 'mefs-keeper config Addresses'.
For example, to change the 'API' port:

  mefs-keeper config Addresses.API /ip4/127.0.0.1/tcp/5001
...
other computers in the network, use 0.0.0.0 as the ip address:
  ...
  mefs-keeper config Addresses.API /ip4/0.0.0.0/tcp/5001
...
mefs-keeper supports passing arbitrary headers to the API and Gateway. You can
do this by setting headers on the API.HTTPHeaders key:

  mefs-keeper config --json API.HTTPHeaders.X-Special-Header '["so special :)"]'
...
You can setup CORS headers the same way:
  ...
  mefs-keeper config --json API.HTTPHeaders.Access-Control-Allow-Origin '["example.com"]'
  mefs-keeper config --json API.HTTPHeaders.Access-Control-Allow-Methods '["PUT", "GET", "POST"]'
  mefs-keeper config --json API.HTTPHeaders.Access-Control-Allow-Credentials '["true"]'
...
`,
	...
	Options: []cmds.Option{
		cmds.BoolOption(initOptionKwd, "Initialize mefs-keeper with default settings if not already initialized"),
