...
$(d)-install: GOFLAGS += $(cmd/mefs-user_flags)
$(d)-install: $(d) $$(DEPS_GO) ALWAYS 
	$(GOCC) install $(go-flags-with-tags) ./cmd/mefs-user
.PHONY: $(d)-install
...
$(COVER_BIN_$(d)): $(d) $$(DEPS_GO) ALWAYS
	$(eval TMP_PKGS := $(shell $(GOCC) list -f '{{range .Deps}}{{.}} {{end}}' $(go-flags-with-tags) ./cmd/mefs-user | sed 's/ /\n/g' | grep mefs/go-mefs) $(call go-pkg-name,$<))
