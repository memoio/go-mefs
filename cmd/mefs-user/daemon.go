...
var daemonCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		...
		ShortDescription: `
'mefs-user daemon' runs a persistent mefs-user daemon that can serve commands
over the network. Most applications that use MEFS will do so by
communicating with a daemon over the HTTP API. While the daemon is
running, calls to 'mefs-user' commands will be sent over the network to
the daemon.
`,
		LongDescription: `
The daemon will start listening on ports on the network, which are
documented in (and can be modified through) 'mefs-user config Addresses'.
For example, to change the 'API' port:

  mefs-user config Addresses.API /ip4/127.0.0.1/tcp/5001
...
mefs-user supports passing arbitrary headers to the API and Gateway. You can
do this by setting headers on the API.HTTPHeaders key:

  mefs-user config --json API.HTTPHeaders.X-Special-Header '["so special :)"]'
...
You can setup CORS headers the same way:
  ...
  mefs-user config --json API.HTTPHeaders.Access-Control-Allow-Origin '["example.com"]'
  mefs-user config --json API.HTTPHeaders.Access-Control-Allow-Methods '["PUT", "GET", "POST"]'
  mefs-user config --json API.HTTPHeaders.Access-Control-Allow-Credentials '["true"]'
...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) error {
	...
	if !cfg.Test {
		...
		if err != nil {
			utils.MLogger.Error("judge account's role err: ", err)
			return err
		}
		if isKeeper {
