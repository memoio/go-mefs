...
import (
	...
	"github.com/memoio/go-mefs/source/goleveldb/leveldb/cache"
...
func (r *Reader) newBlockIter(b *block, bReleaser util.Releaser, slice *util.Range, inclLimit bool) *blockIter { //从这里可以看出是在*block里面查找匹配的
	bi := &blockIter{
		tr:            r,
		block:         b,
...
func (r *Reader) NewIterator(slice *util.Range, ro *opt.ReadOptions) iterator.Iterator {
	...
	fillCache := !ro.GetDontFillCache() //true
	indexBlock, rel, err := r.getIndexBlock(fillCache)
	if err != nil {
		return iterator.NewEmptyIterator(err)
	}
	index := &indexIter{
		blockIter: r.newBlockIter(indexBlock, rel, slice, true), //在indexBlock中找到
		tr:        r,
		slice:     slice,
