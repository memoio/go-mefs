...
func main() {
	app := cli.NewApp()
	app.Usage = "metb is a tool for managing test clusters of mefs nodes"
...
var initCmd = cli.Command{
	...
	Flags: []cli.Flag{
		cli.IntFlag{
			Name:  "count, n",
			Usage: "number of mefs nodes to initialize",
		},
		cli.IntFlag{
			Name:  "port, p",
...
var startCmd = cli.Command{
	...
	Flags: []cli.Flag{
		...
		cli.StringFlag{
			Name:  "args",
			Usage: "extra args to pass on to the mefs daemon",
		},
	},
	Action: func(c *cli.Context) error {
		var extra []string
		...
		// config, err := ioutil.ReadFile("/home/lhy/testbed/0/config")
		// if err != nil {
		// 	log.Fatal(err)
		// }
		// keystore, err := ioutil.ReadFile("/home/lhy/testbed/0/keystore/peerPrivateKey")
		// if err != nil {
		// 	log.Fatal(err)
		// }
		// lnode, err := gateway.InitUser(config, keystore)
		// if err != nil {
		// 	return nil
		// }
		// gateway.Start(lnode, nil)
		// fmt.Println("c.args():", c.Args())
		// if len(c.Args()) != 0 {
		// 	err := gateway.KillUser(lnode)
		// 	if err != nil {
		// 		return nil
		// 	}
		// }
...
var restartCmd = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		err = util.IpfsKillAll(nodes)
		if err != nil {
			return fmt.Errorf("mefs kill err: %s", err)
		}

		err = util.IpfsStart(nodes, c.Bool("wait"), nil)
		handleErr("mefs start err: ", err)
		return nil
	},
}
...
MEFS_PATH - set to testbed node 'n's MEFS_PATH
...
var connectCmd = cli.Command{
	...
	Action: func(c *cli.Context) error {
		if len(c.Args()) < 2 {
			fmt.Println("metb connect [node] [node]")
			os.Exit(1)
		}
...
var dumpStacksCmd = cli.Command{
	...
	Action: func(c *cli.Context) error {
		if len(c.Args()) < 1 {
			fmt.Println("metb dump-stack [node]")
			os.Exit(1)
		}
