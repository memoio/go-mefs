...
_ipfs_daemon()
{
    ...
    elif [[ ${prev} == "--mount-mefs" ]] || [[ ${prev} == "--mount-ipns" ]] || [[ ${prev} == "=" ]]; then
        _ipfs_filesystem_complete
    elif [[ ${word} == -* ]] ; then
        _ipfs_comp "--init --routing= --mount --writable --mount-mefs= \
...
_ipfs_diag_cmds()
{
    if [[ ${prev} == "clear" ]] ; then
        ...
        _ipfs_comp "ns us µs ms s m h" # TODO: Trigger with out space, eg. "mefs diag set-time 10ns" not "... set-time 10 ns"
...
_ipfs_mount()
{
    if [[ ${prev} == "--mefs-path" ]] || [[ ${prev} == "--ipns-path" ]] || [[ ${prev} == "=" ]] ; then
        _ipfs_filesystem_complete
    elif [[ ${word} == -* ]] ; then
        _ipfs_comp "--mefs-path= --ipns-path= --help"
    fi
}
...
_ipfs_name_publish()
{
    if [[ ${prev} == "--lifetime" ]] || [[ ${prev} == "--ttl" ]] ; then
        ...
        _ipfs_comp "ns us µs ms s m h" # TODO: Trigger without space, eg. "mefs diag set-time 10ns" not "... set-time 10 ns"
    elif [[ ${word} == -* ]] ; then
...
_ipfs_refs()
{
    ...
    #else
        # TODO: Use "mefs ref" and combine it with autocomplete, see _ipfs_hash_complete
    fi
}
...
_ipfs_swarm_disconnect()
{
    ...
    opts=$(for x in `mefs swarm peers`; do echo ${x} ; done)
...
_ipfs_update_install()
{
    if   [[ ${prev} == v*.*.* ]] ; then
        ...
        local OLDIFS="$IFS" ; local IFS=$'\n' # Change divider for iterator one line below
        opts=$(for x in `mefs update versions`; do echo ${x} ; done)
        IFS="$OLDIFS" # Reset divider to space, ' '
        COMPREPLY=( $(compgen -W "${opts}" -- ${word}) )
    fi
}
...
{
    local lastDir=${word%/*}/
    echo "LastDir: ${lastDir}" >> ~/Downloads/debug-mefs.txt
    local OLDIFS="$IFS" ; local IFS=$'\n' # Change divider for iterator one line below
    opts=$(for x in `mefs file ls ${lastDir}`; do echo ${lastDir}${x}/ ; done) # TODO: Implement "mefs file ls -F" to get rid of frontslash after files. This take long time to run first time on a new shell.
    echo "Options: ${opts}" >> ~/Downloads/debug-mefs.txt
    IFS="$OLDIFS" # Reset divider to space, ' '
    echo "Current: ${word}" >> ~/Downloads/debug-mefs.txt
    COMPREPLY=( $(compgen -W "${opts}" -- ${word}) )
    echo "Suggestion: ${COMPREPLY}" >> ~/Downloads/debug-mefs.txt
...
_ipfs_files_complete()
{
    local lastDir=${word%/*}/
    local OLDIFS="$IFS" ; local IFS=$'\n' # Change divider for iterator one line below
    opts=$(for x in `mefs files ls ${lastDir}`; do echo ${lastDir}${x}/ ; done) # TODO: Implement "mefs files ls -F" to get rid of frontslash after files. This does currently throw "Error: /cats/foo/ is not a directory"
    IFS="$OLDIFS" # Reset divider to space, ' '
    COMPREPLY=( $(compgen -W "${opts}" -- ${word}) )
    [[ $COMPREPLY = */ ]] && compopt -o nospace -o filenames
    return 0
