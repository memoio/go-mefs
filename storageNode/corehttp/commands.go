...
cli arguments:
	...
	mefs config API.HTTPHeaders --json '{"Access-Control-Allow-Origin": ["*"]}'
	mefs daemon
`
...
func addHeadersFromConfig(c *cmdsHttp.ServerConfig, nc *config.Config) {
	...
	c.Headers["Server"] = []string{"go-mefs/" + version.CurrentVersionNumber}
}
...
func commandsOption(cctx oldcmds.Context, command *cmds.Command) ServeOption {
	return func(n *core.MefsNode, l net.Listener, mux *http.ServeMux) (*http.ServeMux, error) {
		...
		parent.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
			if strings.HasPrefix(r.URL.Path, APIPath) {
				...
				if len(pth) >= 2 && pth[1] != "version" {
					...
					// skips check if client is not go-mefs
					if strings.Contains(clientVersion, "/go-mefs/") && daemonVersion != clientVersion {
