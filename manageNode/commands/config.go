...
var ConfigCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		Tagline: "Get and set mefs config values.",
		ShortDescription: `
'mefs config' controls configuration variables. It works like 'git config'.
The configuration values are stored in a config file inside your mefs
repository.`,
		LongDescription: `
'mefs config' controls configuration variables. It works
much like 'git config'. The configuration values are stored in a config
file inside your MEFS repository.

Examples:

Get the value of the 'Datastore.Path' key:

  $ mefs config Datastore.Path
...
`,
	...
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		...
		if len(args) == 2 {
			...
			case "MarketingMoney":
				mm, err := strconv.ParseInt(value, 10, 0)
				if err != nil {
					return err
				}
				keeper.MarketingMoney = mm
			default:
			}
...
var configEditCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		...
		ShortDescription: `
To use 'mefs config edit', you must have the $EDITOR environment
...
var configProfileApplyCmd = &cmds.Command{
	...
	Options: []cmds.Option{
		cmds.BoolOption("dry-run", "print difference between the current config and the config that would be generated"),
	},
	Arguments: []cmds.Argument{
