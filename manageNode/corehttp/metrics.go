...
func MetricsCollectionOption(handlerName string) ServeOption {
	return func(_ *core.MefsNode, _ net.Listener, mux *http.ServeMux) (*http.ServeMux, error) {
...
var (
	peersTotalMetric = prometheus.NewDesc(
		prometheus.BuildFQName("mefs", "p2p", "peers_total"),
		"Number of connected peers", []string{"transport"}, nil)
)

type MefsNodeCollector struct {
	Node *core.MefsNode
}

func (_ MefsNodeCollector) Describe(ch chan<- *prometheus.Desc) {
	ch <- peersTotalMetric
}

func (c MefsNodeCollector) Collect(ch chan<- prometheus.Metric) {
...
func (c MefsNodeCollector) PeersTotalValues() map[string]float64 {
