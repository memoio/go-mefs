...
func benchmarkEncodeFile(b *testing.B, dataShards, parityShards, fullDataSize int) {
	shardSize := (fullDataSize-1)/dataShards + 1
	r, err := New(dataShards, parityShards, WithAutoGoroutines(shardSize))
	if err != nil {
		b.Fatal(err)
	}
...
// func benchmarkEncodeFile(b *testing.B, dataShards, parityShards int) {
// 	r, err := New(dataShards, parityShards)
// 	if err != nil {
// 			b.Fatal(err)
// 		}

// 	date := make([]byte, 1024*1024*1024)

// 	rand.Seed(0)
// 	fillRandom(date)

// 	b.SetBytes(int64(1024*1024*1024))
// 	b.ResetTimer()
// 	for i := 0; i < b.N; i++ {
// 		shards, err := r.Split(date)
// 		if err != nil {
// 			b.Fatal(err)
// 		}
// 		err = r.Encode(shards)
...
func BenchmarkEncode100x10x256KB(b *testing.B) {
	benchmarkEncodeFile(b, 5, 5, 2*1024*1024*1024)
}
...
func benchmarkReconstructFile(b *testing.B, dataShards, parityShards, fullDataSize int) {
	shardSize := (fullDataSize-1)/dataShards + 1
	r, err := New(dataShards, parityShards, WithAutoGoroutines(shardSize))
	if err != nil {
		b.Fatal(err)
...
func BenchmarkReconstructFile(b *testing.B) {
	benchmarkReconstructFile(b, 5, 2, 2*1024*1024*1024)
}
func BenchmarkReconstruct10x2x10000(b *testing.B) {
	benchmarkReconstruct(b, 20, 10, 512*1024)
}
...
func benchmarkReconstructFileP(b *testing.B, dataShards, parityShards, fullDataSize int) {
	shardSize := (fullDataSize-1)/dataShards + 1
	r, err := New(dataShards, parityShards, WithAutoGoroutines(shardSize))
	if err != nil {
		b.Fatal(err)
