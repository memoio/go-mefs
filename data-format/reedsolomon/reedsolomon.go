...
type Encoder interface {
	...
	Encode(shards [][]byte) error // 编码数据块与冗余块

	// Verify returns true if the parity shards contain correct data.
	// The data is the same format as Encode. No data is modified, so
	// you are allowed to read from data while this is running.
	Verify(shards [][]byte) (bool, error) //	检查冗余块是否含有正确的数据
	...
	Reconstruct(shards [][]byte) error // 根据剩余的数据块和冗余块重新组合数据块或冗余块
	...
	ReconstructData(shards [][]byte) error // 根据剩余的数据块和冗余块重新组合数据块
	...
	Update(shards [][]byte, newDatashards [][]byte) error // 在更改少量数据块时更新冗余块值，比encoder快。适用数据块大、更改少的情况
	...
	Split(data []byte) ([][]byte, error) // 切分数据成数据块+空冗余的结构
...
func (r reedSolomon) Split(data []byte) ([][]byte, error) {
	...
	// perShard := len(data) / r.DataShards
