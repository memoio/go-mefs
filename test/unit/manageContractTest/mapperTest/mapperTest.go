...
func main() {
	...
	defer log.Println("============finish test mapper successfully===========")

	log.Println("start deploy resolver")
	_, resolver, err := contracts.DeployResolver(localAddr, userSk, "test")
	if err != nil {
	...
	if mapperAddr != mapperAddrRemote {
		log.Fatal("the mapperAddress different from remote")
	}
}
